...
from VietnameseTextClassification.fileProcess import FileReader, FileStore
...
def train_model(model_name = 'logistic'):
    ...
    if model_name == 'knn':
        ...
        estKNeighbors.save_model(filePath=os.path.join(settings.MODEL_PATH,'knn_model_tfidf.pk')) # save Model
        ...
        estSVM.training()
        estSVM.save_model(filePath=os.path.join(settings.MODEL_PATH,'svm_model.pk')) # save Model
        ...
        estRandomForest.save_model(filePath=os.path.join(settings.MODEL_PATH,'random_forest_model.pk')) # save Model
        ...
        estLogistic.save_model(filePath=os.path.join(settings.MODEL_PATH,'logistic_model.pk')) # save Model
